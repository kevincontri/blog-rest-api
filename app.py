...
def create_post(post: PostCreate, current_user_id: str = Depends(get_current_user)):
    ...
    raise HTTPException(
        status_code=400,
        detail="Bad Request Body - Author ID not found"
    )
...
def update_post(post_id: str, post: PostUpdate, current_user_id: str = Depends(get_current_user)):
    service = PostService()
...
def create_comment(post_id: str, comment: CommentCreate, current_user_id: str = Depends(get_current_user)):
    ...
    raise HTTPException(
        status_code=404,
        detail="User and/or post not found"
    )
